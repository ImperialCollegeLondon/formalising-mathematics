...
open neighbourhood `𝓝 x` of `x` have nonempty intersection. Let's go through
the notation more carefully though. Recall that the order on filters is
...
So this boils down to saying that there do not exists sets `A ∈ 𝓝 x` and `B ∈ F`
