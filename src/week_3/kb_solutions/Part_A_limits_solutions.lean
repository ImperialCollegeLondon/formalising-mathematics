...
begin
  by_cases hc : c = 0,
  { -- if c = 0 then the sequence is 0
    subst hc, -- this means "rewrite `c = 0` everywhere and then delete `c`"
    -- so this should be the proof:
    convert is_limit_const 0,
    -- and now we tidy up
    { ext, simp },
    { simp } },

  { -- If c ≠ 0 then we can use `abs_pos`. Note `rwa` means `rw, then assumption`.
    have hc' : 0 < |c| := by rwa abs_pos,
    -- Say ε>0.
    intros ε hε,
    -- Then ε/|c|>0
    have hεc : 0 < ε / |c| := div_pos hε hc',
    -- so choose `N` such that `n ≥ N` implies a(n) is within ε/|c| of a
